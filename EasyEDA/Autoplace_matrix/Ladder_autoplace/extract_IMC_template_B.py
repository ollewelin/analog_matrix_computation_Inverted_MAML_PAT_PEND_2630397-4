#!/usr/bin/env python3
"""
Extract new template B from Inverted_MAML_IMC.epru
Search area: X: 12.7-25mm, Y: 225-321mm
This template has 2-level hierarchy: designator$CBB17 (no deeper level like /CBB1)
Move anchor X -100mm
"""
...
def main():
    print("=" * 120)
    print("EXTRACT TEMPLATE B FROM Inverted_MAML_IMC.epru")
    print("=" * 120)
    ...
    # Search area: X 12.7-25mm, Y 225-321mm
    x_min, x_max, y_min, y_max = 12.7, 25, 225, 321
    ...
    # Apply X -100mm offset to anchor
    anchor_x_mm = anchor_x_mm_orig - 100.0
    
    print(f"\nAnchor point: {anchor_comp['designator']} at X={anchor_x_mm_orig:.3f}mm, Y={anchor_y_mm:.3f}mm")
    print(f"After X-offset (-100mm): X={anchor_x_mm:.3f}mm, Y={anchor_y_mm:.3f}mm")
    
    # Build template dictionary with 2-level hierarchy (no /CBB suffix)
    template_dict = {}
    for comp in found:
        ...
        # Calculate relative positions with X-offset
        rel_x = comp['x_mm'] - anchor_x_mm_orig - 100.0
    ...
    # Save template
    template_file = 'IMC_template_B_x_y_rot.txt'
    with open(template_file, 'w') as f:
        f.write("# Template B - 2-level hierarchy\n")
    ...
    dict_file = 'IMC_template_B_dict.py'
    with open(dict_file, 'w') as f:
        f.write("# IMC Template B Dictionary (2-level hierarchy)\n")
        f.write(f"# Anchor: {anchor_comp['designator']} at X={anchor_x_mm:.3f}mm, Y={anchor_y_mm:.3f}mm\n\n")
        f.write("TEMPLATE_IMC_B = {\n")
        for des, (rel_x, rel_y, angle) in sorted(template_dict.items()):
