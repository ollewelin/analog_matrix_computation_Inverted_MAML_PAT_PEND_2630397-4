...
targets = ['matrix3', 'matrix8', 'matrix33']
for rb in targets:
    if rb not in blocks: continue
    print(f"Reuse Block: {rb}")
    locked_prefixes = []
    for p in blocks[rb]:
        if any(c['locked'] for c in blocks[rb][p]):
            locked_prefixes.append(p)
    
    for p in sorted(locked_prefixes):
        xs = [c['x'] for c in blocks[rb][p]]
        ys = [c['y'] for c in blocks[rb][p]]
        avg_x = sum(xs)/len(xs)
        avg_y = sum(ys)/len(ys)
        print(f"  Locked Prefix: {p}, AvgPos: ({avg_x:.2f}, {avg_y:.2f})")
