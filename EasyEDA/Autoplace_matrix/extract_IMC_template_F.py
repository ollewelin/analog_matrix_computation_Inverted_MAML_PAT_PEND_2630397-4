#!/usr/bin/env python3
"""
Extract new template F from Inverted_MAML_IMC.epru - BOTTOM LAYER
                ...
                try:
                    ...
                    if meta.get('type') == 'COMPONENT':
                        ...
                        components[comp_id] = {
                            ...
                            'layer': data.get('layer', 'Top'),  # Extract layer info
                        }
                except Exception:
                    continue
...
def main():
    print("=" * 120)
    print("EXTRACT TEMPLATE F FROM Inverted_MAML_IMC.epru - BOTTOM LAYER")
    print("=" * 120)
    ...
    # Search area: X 85-99mm, Y 10-27mm (from image coordinates)
    x_min, x_max, y_min, y_max = 85, 99, 10, 27
    ...
    print(f"Anchor layer: {anchor_comp.get('layer', 'Top')}")
    ...
    # Save template
    template_file = 'IMC_template_F_x_y_rot.txt'
    with open(template_file, 'w') as f:
        f.write("# Template F - First-level hierarchy (BOTTOM LAYER)\n")
        f.write(f"# Anchor: {anchor_comp['designator']} at X={anchor_x_mm_orig:.3f}mm, Y={anchor_y_mm:.3f}mm\n")
        f.write(f"# Layer: {anchor_comp.get('layer', 'Top')}\n\n")
        for des, (rel_x, rel_y, angle) in sorted(template_dict.items()):
            layer = layer_info.get(des, 'Top')
    ...
    dict_file = 'IMC_template_F_dict.py'
    with open(dict_file, 'w') as f:
        f.write("# IMC Template F Dictionary (first-level hierarchy, BOTTOM LAYER)\n")
        f.write(f"# Anchor: {anchor_comp['designator']} at X={anchor_x_mm_orig:.3f}mm, Y={anchor_y_mm:.3f}mm\n\n")
        f.write("TEMPLATE_IMC_F = {\n")
        for des, (rel_x, rel_y, angle) in sorted(template_dict.items()):
