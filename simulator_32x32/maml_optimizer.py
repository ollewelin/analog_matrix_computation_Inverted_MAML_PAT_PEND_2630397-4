...
class InvertedMAML:
    ...
    def __init__(self, triad: AtomicTriad, learning_rate: float = 0.05,
                 ...
                 adaptive_lr: bool = False, lr_decay_factor: float = 0.95):
        """
        ...
        Args:
            ...
            learning_rate: Gradient descent step size (0.05 default, 0.5 for adaptive)
            ...
            adaptive_lr: Enable adaptive learning rate decay (False=constant, True=exponential decay)
            lr_decay_factor: Learning rate decay per 10 cycles (0.95 = 5% decay per 10 cycles)
        """
        self.triad = triad
        self.lr_init = learning_rate  # Store initial learning rate
        self.lr = learning_rate        # Current learning rate
    ...
    def compute_stratified_gradient(self, x_input: np.ndarray, 
        ...
        for stratum in range(num_early_strata):
            ...
            # Loss at this stratum: L = MSE(output, target) = mean(error²)
    ...
    def _backprop_corrections(self, error: np.ndarray, diagnostics: Dict,
        ...
        # Backprop through scaling: ∂L/∂(M3@x) = ∂L/∂(scaled) x 7.0
    ...
    def update_weights(self, x_input: np.ndarray, y_target: np.ndarray) -> float:
        ...
        if self.adaptive_lr:
            self.lr = self.lr_init * (self.lr_decay_factor ** (self.cycle_count / 10.0))
...
def create_test_vectors(num_vectors: int = 16, dimension: int = 32, 
    ...
    Args:
        ...
        dimension: Vector dimension (32 for 32x32 matrices)
        seed: Random seed for reproducibility
    
    Returns:
        x_test: (num_vectors, 32) input vectors
        y_test: (num_vectors, 32) target outputs
    """
    np.random.seed(seed)
