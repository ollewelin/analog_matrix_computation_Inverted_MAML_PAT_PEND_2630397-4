...
class AnalogCell:
    ...
    def __init__(self, cell_id: int, V_th: float = 0.6, g_m_scale: float = 1.0e-2,
                 ...
                 v_sat_param: float = 0.15):
        """Initialize an analog cell.
        
        Args:
            v_sat_param: Velocity saturation parameter (0.0 = no saturation, 0.15 = 180nm realistic)
        """
        ...
        # Per-cell Vth OFF variation (Gaussian spread from batch variation)
        # Each transistor in the batch has slightly different Vth
        self.V_th_variation_offset = 0.0
    ...
    def compute_output(self, V_ds: float) -> float:
        """
        Compute output current in triode region with velocity saturation.
        
        I_out = g_m_eff · (V_gs - V_th - V_ds/2) · V_ds
        
        where g_m_eff = g_m / (1 + v_sat_param · V_ds / V_crit)
        ...
        Note:
            - If triode condition violated, returns minimal current
            - Velocity saturation reduces transconductance at high V_ds (realistic 180nm effect)
            - Per-cell Vth variation causes transistors to fall outside triode region differently
        """
        # Effective parameters with manufacturing variation AND per-cell Vth offset
        # This models realistic batch variation: each transistor has different Vth OFF
        V_th_eff = self.V_th_mfg + self.V_th_variation_offset
        g_m_base = self.g_m_mfg
        ...
        if V_ds > V_gs_eff:
            ...
            # Transistor operating outside triode region (realistic batch variation effect)
            I_out = g_m_base * 1e-8
        else:
            # VELOCITY SATURATION: Reduce g_m at high electric fields
            # E_field ∝ V_ds / L_channel; normalized to critical field at V_crit = 0.2V
            V_crit = 0.2  # Critical field reference voltage (200mV)
            E_field_normalized = V_ds / V_crit
            g_m_eff = g_m_base / (1.0 + self.v_sat_param * E_field_normalized)
            
            # Triode region (with velocity saturation): I_out ∝ (V_gs - V_th - V_ds/2) · V_ds
    ...
    def get_state(self) -> Dict[str, float]:
        ...
        return {
            'V_gs': self.V_gs,
            'V_th': self.V_th_mfg + self.V_th_variation_offset,  # Include per-cell offset
            'tau_discharge': self.tau_discharge,
            'temperature_C': self.temperature_delta_C,
            'triode_margin': self.V_gs - (self.V_th_mfg + self.V_th_variation_offset),
            'V_th_variation_offset': self.V_th_variation_offset  # Track per-cell offset
        }
    
    def set_vth_variation(self, variation: float):
        """
        Set per-cell Vth OFF variation offset.
        
        This allows realistic batch-to-batch variation where each transistor
        has a slightly different threshold voltage. Some transistors may fall
        outside the triode region with certain weight settings.
        
        Args:
            variation: Vth offset in Volts (e.g., -0.05 to +0.05 for ±5% variation)
        """
        self.V_th_variation_offset = variation
...
class CellBank:
    ...
    def __init__(self, num_active: int = 36, num_reference: int = 6, num_bias: int = 6, 
        ...
        Args:
            ...
            v_sat_param: Velocity saturation parameter (0.0 = ideal, 0.15 = 180nm realistic)
        """
        self.cells_active = [AnalogCell(i, v_sat_param=v_sat_param) for i in range(num_active)]
    ...
    def apply_per_cell_vth_variations(self, vth_variations: np.ndarray):
        """
        Apply per-cell Vth OFF variations to active and bias cells.
        
        This implements realistic batch-to-batch transistor variation where each
        individual cell has a unique threshold voltage offset. This causes different
        compression ratios and some transistors to fall outside triode region.
        
        Args:
            vth_variations: 1D or 2D numpy array of Vth offsets
                - If 2D (e.g., 32x32): flattened to match cells_active
                - If 1D: should have length matching cells_active + cells_bias
                - Values typically ±5-20% of V_th (e.g., ±0.03-0.12V for 0.6V base)
        
        Example:
            # Generate realistic ±10% Vth variation
            vth_vars = np.random.normal(0, 0.06, size=(32, 32))
            cell_bank.apply_per_cell_vth_variations(vth_vars)
        """
        vth_vars_flat = vth_variations.flatten()
        
        # Apply to active cells
        num_active = len(self.cells_active)
        for i, cell in enumerate(self.cells_active):
            if i < len(vth_vars_flat):
                cell.set_vth_variation(vth_vars_flat[i])
        
        # Apply to bias cells (use additional variations or recycle)
        num_bias = len(self.cells_bias)
        for i, cell in enumerate(self.cells_bias):
            if num_active + i < len(vth_vars_flat):
                cell.set_vth_variation(vth_vars_flat[num_active + i])
            else:
                # Generate fresh variations for bias cells if needed
                cell.set_vth_variation(np.random.normal(0, vth_vars_flat.std()) if len(vth_vars_flat) > 0 else 0.0)
