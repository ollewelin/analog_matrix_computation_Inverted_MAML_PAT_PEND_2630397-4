"""Matrix Core Layer: Atomic Triad (3 x 32x32 matrices) + Computation Model
Implements the core Inverted MAML architecture (SCALED 32x32):
    - M33: Primary payload matrix 32x32 (fixed)
    - M3: First correction layer 32x32 (adaptive via MAML)
    - M8: Second correction layer 32x32 (adaptive via MAML)
    - M3/M8: Include 1 row bias cells + 1 row reference cells

Reference: inverterad_MAML_12_Eng.pdf, Atomic_Triad.pdf
Scale: 6x6 (36 cells) -> 32x32 (1024 cells)
"""
...
class AnalogMatrix:
    """
    Single 32x32 analog matrix with physical cell-level modeling (SCALED).
    
    Structure:
        - Weight storage: 1024 cells (32x32) storing gate voltages
        - Reference cells: 32 fast-decay cells for calibration clock (1 row)
        - Bias cells: 32 cells for hardware bias term (1 row)
    
    Computation: I_out[i] = Σⱼ g_m(V_gs[i,j], V_ds[j]) · (V_ds[j] - V_s)
                where V_s = 1.65V (virtual ground)
    """
    
    def __init__(self, matrix_id: int, size: int = 32, is_correction: bool = False, v_sat_param: float = 0.15):
        """
        Initialize a 32x32 analog matrix (SCALED).
        
        Args:
            matrix_id: Identifier (3=payload, 8=2nd correction, 33=primary)
            size: Matrix dimension (default 32x32)
            is_correction: If True, weights are trainable (MAML)
            v_sat_param: Velocity saturation parameter (0.15 = 180nm realistic)
        """
    ...
    def forward(self, x_input: np.ndarray) -> Tuple[np.ndarray, Dict]:
        """
        Compute matrix-vector product with physical cell modeling (SCALED 32x32).
        
        Args:
            x_input: (32,) input vector, normalized to [0, 1] → [1.65V, 1.90V]
        
        Returns:
            output: (32,) output current vector
    ...
    def apply_per_cell_vth_variations(self, vth_variation_sigma: float = 0.06):
        """
        Apply realistic per-cell Vth OFF variations to all cells in this matrix.
        
        This implements batch-to-batch variation where each transistor has a 
        unique threshold voltage. Some transistors will fall outside triode region,
        creating realistic compression behavior with different compression ratios
        per cell.
        
        Args:
            vth_variation_sigma: Standard deviation of Vth variations in Volts
                Default 0.06 (~10% of 0.6V base) creates realistic batch spread
                - 0.03 = ±5% variation (tight tolerance)
                - 0.06 = ±10% variation (typical batch)
                - 0.12 = ±20% variation (loose tolerance)
        """
        # Generate per-cell Vth variations for this matrix size
        # Each cell gets unique offset from Gaussian distribution
        vth_variations = np.random.normal(0, vth_variation_sigma, size=(self.size, self.size))
        
        # Apply to cell bank
        self.cell_bank.apply_per_cell_vth_variations(vth_variations)
...
class AtomicTriad:
    ...
    def __init__(self, size: int = 6, v_sat_param: float = 0.15):
        """
        Initialize atomic triad.
        
        Args:
            size: Matrix dimension (default 6x6)
            v_sat_param: Velocity saturation parameter (0.15 = 180nm realistic)
        """
        self.size = size
        self.v_sat_param = v_sat_param
        
        # Three matrices with velocity saturation
        self.M33 = AnalogMatrix(matrix_id=33, size=size, is_correction=False, v_sat_param=v_sat_param)
        self.M3 = AnalogMatrix(matrix_id=3, size=size, is_correction=True, v_sat_param=v_sat_param)
        self.M8 = AnalogMatrix(matrix_id=8, size=size, is_correction=True, v_sat_param=v_sat_param)
    ...
    def forward(self, x_input: np.ndarray, t_snapshot_ms: Optional[float] = None) -> Tuple[np.ndarray, Dict]:
        ...
        output = y_payload + y_correction  # M3/M8 enabled
    ...
    def apply_per_cell_vth_variations(self, vth_variation_sigma: float = 0.06):
        """
        Apply realistic per-cell Vth OFF variations to all matrices in the triad.
        
        This creates batch-to-batch variation where each transistor has a unique
        threshold voltage, causing realistic non-uniform behavior across the
        32x32 matrices. Some cells will fall outside triode region, creating
        natural compression with different ratios per cell.
        
        Args:
            vth_variation_sigma: Standard deviation of Vth variations
                - 0.03 = ±5% variation (tight batch)
                - 0.06 = ±10% variation (typical batch) [default]
                - 0.12 = ±20% variation (loose batch, very realistic)
        """
        for matrix in self.ref_matrices:
            matrix.apply_per_cell_vth_variations(vth_variation_sigma)
