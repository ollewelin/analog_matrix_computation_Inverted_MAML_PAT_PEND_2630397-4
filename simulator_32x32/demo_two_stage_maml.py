"""
COMPREHENSIVE TWO-STAGE MAML DEMONSTRATION
===========================================

This script demonstrates:

STAGE 1: Base Model Training
  ✓ Outer Loop: Creates NEW physics each iteration (abrupt changes)
  ✓ Inner Loops: MAML learns to quickly adapt to each physics space
  ✓ Meta-Learning: Base model learns "good starting weights"
  ✓ Result: After K outer iterations, base model can adapt fast to ANY physics

STAGE 2: Operation Mode  
  ✓ Inner Loop Only: Uses learned base model
  ✓ Gradual Drift: Physics changes slowly (thermal, aging)
  ✓ Continuous Adaptation: Inner loop keeps weights optimal
  ✓ Result: Precision maintained despite slow physics degradation

Both stages are visualized with detailed plots showing precision trajectories.
"""
...
def run_two_stage_maml_demo():
    """
    Run complete two-stage MAML demonstration with comprehensive plots.
    """
    
    print("\n" + "=" * 100)
    print("TWO-STAGE META-LEARNING (MAML) DEMONSTRATION")
    print("=" * 100)
    
    # ==================== SETUP ====================
    print("\n[SETUP] Initializing two-stage MAML system...")
    ...
    trainer = TwoStageDynamicMAML(
        ...
        outer_lr=0.01,             # Meta-learning rate (unused in current impl)
        num_strata=1,
        adaptive_lr=True
    )
    
    print(f"  ✓ Created TwoStageDynamicMAML")
    print(f"  ✓ System: 32×32 matrix (1024 cells per matrix × 3 = 3072 cells)")
    ...
    x_train, y_train = create_test_vectors(num_vectors=num_train_samples, dimension=32, seed=42)
    x_test, y_test = create_test_vectors(num_vectors=num_test_samples, dimension=32, seed=43)
    
    print(f"  ✓ Training data: {num_train_samples} vectors")
    print(f"  ✓ Test/Operation data: {num_test_samples} vectors")
    
    # ==================== STAGE 1: BASE MODEL TRAINING ====================
    print("\n" + "=" * 100)
    print("STAGE 1: BASE MODEL TRAINING WITH OUTER LOOP")
    print("=" * 100)
    print("\nConcept:")
    ...
    output_dir = Path("results_32x32/two_stage_maml")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    print("\n[PLOT 1] Base Model Training...")
    fig1 = plot_base_model_training(
        training_log,
        output_path=str(output_dir / "01_base_model_training.png")
    )
    
    print("\n[PLOT 2] Operation Mode...")
    fig2 = plot_operation_mode(
        operation_log,
        output_path=str(output_dir / "02_operation_mode.png")
    )
    
    print("\n[PLOT 3] Stage 1 vs Stage 2 Comparison...")
    fig3 = plot_comparison_training_vs_operation(
        training_log,
        operation_log,
        output_path=str(output_dir / "03_stage1_vs_stage2_comparison.png")
    )
    ...
    results_path = output_dir / "two_stage_maml_results.json"
    ...
    # ==================== FINAL SUMMARY ====================
    print("\n" + "=" * 100)
    print("TWO-STAGE MAML DEMONSTRATION COMPLETE")
    print("=" * 100)
    ...
    print("\n📁 Output files:")
    print(f"  ✓ {output_dir / '01_base_model_training.png'}")
    print(f"  ✓ {output_dir / '02_operation_mode.png'}")
    print(f"  ✓ {output_dir / '03_stage1_vs_stage2_comparison.png'}")
