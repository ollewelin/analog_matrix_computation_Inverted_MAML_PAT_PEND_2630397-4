#!/usr/bin/env python3
"""
Simple 32x32 Test with Double Measurement Window
==================================================

Same as simple_test_32x32.py but with num_strata=2 (double window):
...
def run_simple_test_32_double_window(max_cycles=200, verbose=True):
    """
    Run simple learnable task on 32x32 with TWO MEASUREMENT WINDOWS (num_strata=2).
    ...
    if verbose:
        print("="*75)
        print("SIMPLE 32x32 TEST: DOUBLE MEASUREMENT WINDOW (num_strata=2)")
        print("="*75)
        print()
...
def compare_results(result_32):
    ...
    print()
    print("Configuration:")
    print(f"  Measurement window: {result_32['window']}")
...
if __name__ == '__main__':
    result_32 = run_simple_test_32_double_window(max_cycles=200, verbose=True)
    ...
    ax.plot(phase1_cycles, phase1_prec, 's-', linewidth=2, markersize=6, 
            color='blue', label='Phase 1: Training ON (Double Window)', marker='s')
    ...
    ax.set_title('32x32 Simple Rank-1 Task: Double Measurement Window (num_strata=2)', fontsize=14, fontweight='bold')
    ax.grid(True, alpha=0.3)
    ax.legend(loc='best', fontsize=11)
    ax.set_ylim([2.5, 6.5])
    
    os.makedirs('results_32x32', exist_ok=True)
    plt.savefig('results_32x32/simple_task_32x32_double_window.png', dpi=150, bbox_inches='tight')
    print(f"\n✓ Plot saved: results_32x32/simple_task_32x32_double_window.png")
    plt.close()
