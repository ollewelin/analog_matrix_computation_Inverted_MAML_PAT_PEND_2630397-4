#!/usr/bin/env python3
"""
Simple 24x24 Test with Double Measurement Window
==================================================

Same as simple_test_24x24.py but with num_strata=2 (double window):
  - Single window: num_strata=1 measures at t=0.5ms only
  - Double window: num_strata=2 measures at t=0.5ms AND t=1.5ms

This tests if wider measurement window improves convergence.
...
def create_simple_task_24():
    """Create a simple linear task for 24x24."""
    np.random.seed(42)
    
    # Create a simple rank-2 target matrix
    rank_1 = np.random.randn(24)
    rank_2 = np.random.randn(24)
    
    # Task: Given x, predict y = (rank_1 dot x) * rank_2
    #       This is rank-1, very learnable
    
    num_samples = 16
    x_vectors = np.random.uniform(0.1, 0.9, (num_samples, 24))
    
    y_vectors = np.zeros((num_samples, 24))
...
def run_simple_test_24_double_window(max_cycles=200, verbose=True):
    """
    Run simple learnable task on 24x24 with TWO MEASUREMENT WINDOWS (num_strata=2).
    ...
    DIFFERENCE: num_strata=2 measures at 2 time points instead of 1
    """
    
    if verbose:
        print("="*75)
        print("SIMPLE 24x24 TEST: DOUBLE MEASUREMENT WINDOW (num_strata=2)")
        print("="*75)
        print()
        print("Task: Learn y = (w1 · x) * w2  (rank-1 outer product)")
        print("  Input:  24-dim vector x")
        print("  Output: 24-dim vector y")
        print("  Difficulty: EASY (rank-1, highly learnable)")
        print()
        print("Configuration:")
        print("  Measurement Window: num_strata=2 (measures at t=0.5ms AND t=1.5ms)")
        print("  Learning Rate: 0.1 (higher for easy task)")
        print()
        print("Phases:")
    ...
    # Create simple task
    x_train, y_train = create_simple_task_24()
    
    # Initialize with DOUBLE MEASUREMENT WINDOW
    triad = AtomicTriad(size=24)
    maml = InvertedMAML(triad, learning_rate=0.1, num_strata=2)  # DOUBLE WINDOW
    ...
    if verbose:
        print("-"*75)
        print(f"\nResults (Simple Rank-1 Task with Baseline, Double Window):")
    ...
    return {
        ...
        'window': 'double (num_strata=2)'
    }


def compare_results(result_24):
    """Compare baseline vs training phases."""
    print()
    print("="*75)
    print("PHASE ANALYSIS (DOUBLE MEASUREMENT WINDOW)")
    print("="*75)
    print()
    print(f"Phase 0 (Training OFF - Baseline):")
    print(f"  Average precision: {result_24['baseline_avg']:.2f} bits")
    print()
    print(f"Phase 1 (Training ON - Learning):")
    print(f"  Average precision: {result_24['training_avg']:.2f} bits")
    print(f"  Improvement over baseline: +{result_24['training_avg'] - result_24['baseline_avg']:.2f} bits")
    print()
    print(f"Total improvement (Cycle 0 → {len(result_24['precisions'])-1}):")
    print(f"  {result_24['improvement']:+.2f} bits")
    print()
    print("Configuration:")
    print(f"  Measurement window: {result_24['window']}")
    print("  - Measures at t=0.5ms AND t=1.5ms (2 time points)")
    print("  - Provides richer gradient information than single point")
    print()
    print("Notes:")
    print("  - 24x24 has 16.7x more cells than 6x6 (576 vs 36)")
    print("  - Rank-1 task is easier than full 24x24 mapping")
...
if __name__ == '__main__':
    result_24 = run_simple_test_24_double_window(max_cycles=200, verbose=True)
    
    # Plot with two phases highlighted
    fig, ax = plt.subplots(figsize=(14, 7))
    
    cycles = list(range(len(result_24['precisions'])))
    precisions = result_24['precisions']
    phases = result_24['phases']
    ...
    ax.plot(phase1_cycles, phase1_prec, 's-', linewidth=2, markersize=6, 
            color='purple', label='Phase 1: Training ON (Double Window)')
    ...
    ax.axhline(y=result_24['baseline_avg'], color='red', linestyle=':', linewidth=1.5, 
               alpha=0.5, label=f'Baseline Avg: {result_24["baseline_avg"]:.2f} bits')
    
    ax.set_xlabel('Cycle', fontsize=12)
    ax.set_ylabel('Precision (bits)', fontsize=12)
    ax.set_title('24x24 Simple Rank-1 Task: Double Measurement Window (num_strata=2)', fontsize=14, fontweight='bold')
    ax.grid(True, alpha=0.3)
    ax.legend(loc='best', fontsize=11)
    ax.set_ylim([2.5, 6.5])
    
    os.makedirs('results_24x24', exist_ok=True)
    plt.savefig('results_24x24/simple_task_24x24_double_window.png', dpi=150, bbox_inches='tight')
    print(f"\n✓ Plot saved: results_24x24/simple_task_24x24_double_window.png")
    plt.close()
    
    compare_results(result_24)
